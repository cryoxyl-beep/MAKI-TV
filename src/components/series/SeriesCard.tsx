...
export const SeriesCard = React.memo(
  ({ series, index = 0 }: SeriesCardProps) => {
    ...
    const poster = series.poster_path
      ? `${TMDB_IMAGE_BASE_URL_W500}${series.poster_path}`
      : "";
    ...
    const onClick = () => navigate(`/series/show/${series.id}`);
  ...
  (prev, next) => {
    return prev.series.id === next.series.id && prev.index === next.index;
  },
);
