...
export default function HomeFeed({ onSelectAnime, searchQuery = "", onClearSearch }: HomeFeedProps) {
  ...
  const bannerAnimes = !searchQuery && selectedCategory === "All" ? animeList.slice(0, 8) : [];
  const feedAnimes = (!searchQuery && selectedCategory === "All") ? animeList.slice(8) : animeList;

  return (
    <div className="w-full min-h-screen bg-transparent pb-20">
      
      {/* Hero Banner with Swiper */}
      {!searchQuery && selectedCategory === "All" && bannerAnimes.length > 0 && (
      ...
      <div className={searchQuery ? "max-w-6xl mx-auto px-4 md:px-6 py-6 flex flex-col gap-5" : "grid grid-cols-1 sm:grid-cols-2 md:grid-cols-3 lg:grid-cols-4 gap-x-4 gap-y-8 px-4 md:px-6"}>
