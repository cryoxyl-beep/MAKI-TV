...
import { TMDBMovieDetails, getMovieDetails, TMDB_IMAGE_BASE_URL_W500 } from "../../services/tmdb";
...
export default function MovieWatch() {
  ...
  const defaultProvider = getSettings()?.playback?.defaultServerMovies || "vidfast";
  ...
  useEffect(() => {
    const fetchMovie = async () => {
      ...
      try {
        ...
        setMovie(data);
        document.title = `${data.title} • Movie`;
      } catch (error) {
  ...
  const handleSelectProvider = (pid: string) => {
    ...
    storage.update("miyoro_settings", (prev: any) => ({ ...prev, playback: { ...prev?.playback, defaultServerMovies: pid } }));
    setIsServerDropdownOpen(false);
  };
  ...
  return (
    <div className="w-full bg-[#0f0f0f] pb-20 pt-14 select-none z-10 relative animate-fade-in text-[#f1f1f1] min-h-screen">
      <Header
        ...
        breadcrumbs={[
          { label: "Movies", onClick: () => navigate("/movies"), color: accentColor },
          { label: movie.title, onClick: () => navigate(`/movies/movie/${movie.id}`), color: accentColor },
          { label: "Watch" }
        ]}
      />
        ...
        <div className="w-full lg:w-[29%] min-w-0 flex flex-col bg-[#121214] border border-white/5 rounded-2xl p-4 overflow-hidden min-h-[400px] lg:min-h-0">
          ...
          <div className="flex flex-col gap-3 flex-1 overflow-y-auto custom-scrollbar pr-2">
            {movie.recommendations?.results?.slice(0, 10).map(rec => (
              <div 
                key={rec.id}
                onClick={() => navigate(`/movies/movie/${rec.id}`)}
              ...
              >
                ...
                <div className="absolute inset-0 flex flex-col justify-center px-4">
                  <h4 className="text-white font-bold text-sm line-clamp-1 tracking-wide">{rec.title}</h4>
            ...
            {(!movie.recommendations || !movie.recommendations.results || movie.recommendations.results.length === 0) && (
               <div className="py-10 text-center text-white/40 text-sm">No suggested movies.</div>
            )}
          </div>
        </div>
      </div>
      ...
      <div className="max-w-[1700px] mx-auto px-4 lg:px-6 pb-20 flex flex-col lg:flex-row gap-6 items-start mt-6">
        <div className="w-full lg:w-[71%] min-w-0 flex flex-col">
          ...
          <div className="mt-2 flex flex-col gap-3 pb-8 w-full min-w-0">
            <div 
            ...
            <div className="flex flex-col xl:flex-row xl:items-center justify-between py-1 gap-4">
              <div className="flex items-center gap-4">
                ...
                >
                  {movie.poster_path ? (
                    <LazyImage src={`${TMDB_IMAGE_BASE_URL_W500}${movie.poster_path}`} alt={movie.title} className="w-full h-full object-cover hover:scale-105 transition-transform" />
              ...
              <div className="flex items-center gap-2 flex-wrap xl:flex-nowrap">
                <div className="relative flex-shrink-0 z-[100]" ref={serverDropdownRef}>
                  ...
                  >
                    <span>{MOVIE_PROVIDERS.find(p => p.id === selectedProvider)?.label || "Server"}</span>
