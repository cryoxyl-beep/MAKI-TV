...
import { getMovieHistory, formatRelativeDate, MovieHistoryItem } from "../../utils";
import { Trash2, Play } from "lucide-react";
import LazyImage from "../LazyImage";
import { useMoviesData, MovieWatchLaterItem } from "../../hooks/useMoviesData";
...
export default function MoviesLibrary() {
  ...
  const [historyItems, setHistoryItems] = useState<MovieHistoryItem[]>([]);
  const { collections, watchLater, isLoading: libraryLoading, removeFromCollection, toggleWatchLater } = useMoviesData();
  const [isLocalLoading, setIsLocalLoading] = useState(true);

  useEffect(() => {
    setHistoryItems(getMovieHistory());
          ...
          {tabs.map((tab) => {
            ...
            return (
              ...
              >
                ...
                {isActive && (
                  <motion.div
                    layoutId="movies-library-tab-indicator"
                    ...
                    <div className="flex flex-col gap-8">
                      {(Object.entries(
                        historyItems.reduce((acc, item) => {
                          ...
                          return acc;
                        }, {} as Record<string, MovieHistoryItem[]>)
                      ) as [string, MovieHistoryItem[]][]).map(([dateStr, items]) => (
                        <div key={dateStr} className="flex flex-col gap-3">
                          ...
                          <div className="flex flex-col gap-2">
                            {items.map((item, idx) => (
                              <div
                                key={`${item.id}-${idx}`}
                                onClick={() => navigate(`/movies/watch/${item.id}`)}
                        ...
                        {(!collections?.toBinge || collections.toBinge.length === 0) ? (
                          ...
                          <div className="flex gap-4 overflow-x-auto pb-4 [scrollbar-width:none] [-ms-overflow-style:none] [&::-webkit-scrollbar]:hidden snap-x">
                            {collections.toBinge.map((item) => (
                              <div
                                key={item.id}
                                onClick={() => navigate(`/movies/movie/${item.id}`)}
              ...
              {activeTab === "watch_later" && (
                <div>
                  {showLoader ? (
                    ...
                    <div className="py-20 text-center">
                      <p className="text-gray-500 text-sm">No movies saved for later.</p>
                    </div>
                  ) : (
                    <div className="flex flex-col gap-2">
                      {watchLater.map((item) => (
